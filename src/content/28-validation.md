...
Правило активно, если сценарий модели попадает в `on` (или `on` не задан) и не попадает в `except`. Атрибуты активных правил становятся *безопасными* для `load()` — кроме тех, что упомянуты только в правилах с `'safe' => false` или с восклицательным знаком в `scenarios()`.
