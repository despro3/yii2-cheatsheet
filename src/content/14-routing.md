...
'rules' => [
    ...
    // ограничение по HTTP-методу — только для разбора, при генерации такие правила игнорируются
...
Q: Почему правило `'DELETE post/<id:\d+>' => 'post/delete'` не используется при генерации ссылок?
A: Правила с HTTP-методом в шаблоне работают только при разборе входящих запросов.
