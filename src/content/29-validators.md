...
`compare` — сравнить с другим атрибутом `compareAttribute` или значением `compareValue`; `operator` (`==`, `===`, `!=`, `!==`, `>`, `>=`, `<`, `<=`); `type` — как сравнивать: `string` (по умолчанию: оба значения приводятся к строке и сравниваются операторами PHP) или `number` (приводятся к `float`)
...
> [!GOTCHA]
> `compare` по умолчанию приводит оба значения к **строке**. Для чисел это обычно сходит с рук — PHP сравнивает числовые строки как числа, поэтому `'10' > '9'` истинно. А вот на датах в разном формате ломается: `'2024-1-5' > '2024-01-10'` — тоже истина, хотя 5 января раньше 10-го. Для чисел ставьте `'type' => 'number'`; даты так не чинятся (`(float)` от обеих дат даст `2024`) — сравнивайте UNIX-время из `timestampAttribute`.
...
Q: Когда `['end', 'compare', 'compareAttribute' => 'start', 'operator' => '>']` даёт неверный ответ?
A: Когда значения — не просто числа. Сравнение по умолчанию строковое, и `'2024-1-5' > '2024-01-10'` оказывается истиной. Для чисел задайте `'type' => 'number'`, для дат сравнивайте UNIX-время из `timestampAttribute`.
