...
$row   = $query->one();                     // первая строка или false (добавляет LIMIT 1)
...
> [!GOTCHA]
> `one()` **не** проверяет, что строк действительно одна, а просто добавляет `LIMIT 1`. И если в `select()` указаны только выражения без имён столбцов, `indexBy` не найдёт ключ — давайте псевдонимы.
