    ...
    'whenClient' => "function (attribute, value) {
        ...
        ('traps', 'Грабли', [
            ('note', 'trap', 'compare сравнивает строки',
             'По умолчанию оба значения приводятся к строке. На числах это обычно незаметно — PHP сравнивает '
             'числовые строки как числа. Зато ломается на датах в разном формате: `\'2024-1-5\' > \'2024-01-10\'` '
             'истинно, хотя 5 января раньше 10-го. Для чисел ставьте `\'type\' => \'number\'`, для дат сравнивайте '
             'UNIX-время из `timestampAttribute` — приведение к `float` обе даты превратит в `2024`.'),
    ...
    public function __construct($id, $module, BookingInterface $booking, $config = [])
    {
        ...
        ('all', 'Регистрация и разрешение', [
            ...
            ('ref', [
                ...
                {'n': 'set($name, $closure)', 'd': 'Фабрика: полный контроль над созданием. Замыкание получает контейнер, параметры и конфигурацию.', 'o': 'function ($container, $params, $config)', 'c': "Yii::$container->set('search', function (\n    $container, $params, $config\n) {\n    $solr = new SolrService('127.0.0.1');\n    // ...сложная инициализация...\n    return $solr;\n});"},
...
(new yii\web\Application($config))->run();'''),
        ...
        ('all', 'Свойства и псевдонимы', [
            ...
            ('ref', [
                ...
                {'n': 'YII_ENABLE_ERROR_HANDLER', 'd': 'Включать ли обработчик ошибок Yii. Выключают редко — разве что при отладке самого обработчика.', 'o': 'по умолчанию true', 'c': "defined('YII_ENABLE_ERROR_HANDLER')\n    or define('YII_ENABLE_ERROR_HANDLER', false);"},
    ...
    public function actionCreate()
    {
        ...
        ('all', 'Методы и правила', [
            ...
            ('ref', [
                ...
                {'n': 'asJson() · asXml()', 'd': 'Ответ в нужном формате: метод сам выставляет `response->format` и возвращает объект ответа, поэтому его нужно вернуть из действия.', 'o': 'удобно в одном действии из многих', 'c': "return $this->asJson(['ok' => true]);"},
    ...
    // 'enableSession' => false,      // для REST: только токен, без сессии
        ...
        ('all', 'Настройки и правила', [
            ...
            ('ref', [
                ...
                {'n': 'loginRequired()', 'd': 'Отправить гостя на loginUrl, запомнив, куда он шёл. Если клиент по заголовку `Accept` не ждёт HTML — скажем, просит JSON — вместо редиректа бросает 403.', 'o': 'то же делает AccessControl', 'c': "if (Yii::$app->user->isGuest) {\n    return Yii::$app->user->loginRequired();\n}"},
            ...
            ('ref', [
                ...
                {'n': 'roles', 'd': 'Знак ? — гость, @ — любой вошедший. Всё остальное считается ролью или разрешением RBAC и проверяется через can().', 'o': 'можно перечислить несколько', 'c': "['allow' => true, 'actions' => ['update'],\n 'roles' => ['moderator', 'admin']],"},
    ...
    'rules' => [
        ...
        ('all', 'Все виды правил', [
            ...
            ('ref', [
                ...
                {'n': 'Поддомены', 'd': 'Шаблон, в который входит домен. Поддомен читается в параметр и приходит в действие обычным аргументом — так одно приложение обслуживает много сайтов сразу: филиалы, языковые версии, магазины арендаторов.', 'o': 'абсолютный шаблон: `http://`, `https://` или `//` в начале', 'c': "// поддомен читается в параметр — как часть пути\n'http://<city:\\w+>.example.com/<action>' => 'office/<action>',\n// kyiv.example.com/contacts → office/contacts, city = kyiv\n\n// домен целиком, без параметров\n'https://admin.example.com/login' => 'admin/user/login',\n\n// // вместо схемы — протокол останется как у запроса\n'//<lang:(ru|en)>.example.com/posts' => 'post/index',\n\n// в действии это обычный аргумент\npublic function actionContacts($city) { … }\n\n// обратно: домен в правиле — адрес выйдет абсолютным\nUrl::to(['office/contacts', 'city' => 'kyiv']);\n// → http://kyiv.example.com/contacts"},
                {'n': 'yii\\web\\GroupUrlRule', 'd': 'Общая приставка для набора правил — и к адресам, и к маршрутам. Так модуль добавляет свои адреса, не повторяя префикс в каждой строке.', 'o': 'prefix, routePrefix, rules', 'c': "['class' => yii\\web\\GroupUrlRule::class,\n 'prefix' => 'admin',           // приставка к адресам\n 'routePrefix' => 'backend',    // приставка к маршрутам\n 'rules' => [\n     'login' => 'user/login',\n     'posts' => 'post/index',\n ]],\n\n// admin/login → backend/user/login\n// admin/posts → backend/post/index"},
...
$this->redirect(['post/view', 'id' => 7]);'''),
        ...
        ('traps', 'Грабли', [
            ...
            ('note', 'trap', 'Именованных маршрутов в Yii 2 нет',
             'Свойство `name` у правила — только ярлык для отладки: построить по нему адрес нельзя. '
             '`Url::to` всегда получает маршрут и параметры, а менеджер идёт по правилам сверху вниз '
             'и берёт первое, которое умеет собрать такой адрес. Поэтому привычного по Symfony и Laravel '
             '`route(\'post.view\')` здесь не найти — вместо имени адрес задаёт порядок правил.'),
    ...
    'definitions' => [
        ...
        ('all', 'Все 14 встроенных', [
            ('ref', [
                ...
                {'n': 'Обёртки представления', 'd': 'Четыре служебных парных виджета: кэш фрагмента, обёртка другим шаблоном, именованный блок и удаление лишних пробелов.', 'o': 'FragmentCache (через beginCache), Spaceless, ContentDecorator (через beginContent), Block (beginBlock)', 'c': "<?php if ($this->beginCache('menu',\n        ['duration' => 600])) { ?>\n    …тяжёлый блок…\n<?php $this->endCache(); } ?>\n\n<?php $this->beginContent('@app/views/layouts/box.php') ?>\n    содержимое в рамке\n<?php $this->endContent() ?>\n\n<?php $this->beginBlock('sidebar') ?>…<?php $this->endBlock() ?>\n<?= $this->blocks['sidebar'] ?? '' ?>\n\n<?php Spaceless::begin() ?>…<?php Spaceless::end() ?>"},
            ]),
